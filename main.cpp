...
void TRANSACTION::InputTrans(int &Opt, int &ItmList)
{
    ...
    cout << "Enter the transaction type(1/2): ";
    cin >> TransType;

    cout << "Enter the transaction details." << endl;
    cout << "Enter the transaction number: ";
    cin >> TransNo;
    cout << "Enter the transaction name/purpose: ";
    gets(TransName);
    ...
    cout << "Choose from the above(1/2): ";
    cin >> Opt;
...
void main()
{
    ...
    case 1:
        do
        {
            ...
            cout << "Choose as 1/2/3/4/5/6/7/8: ";
            cin >> Process;
...
void InitialiseAccount(float &Budget, float &Income, float &Expenditure)
{
    ...
    cout << "Enter the current budget: ";
    cin >> Budget;
    ACC.write((char *)&Budget, sizeof(Budget));

    cout << "Enter the current income: ";
    cin >> Income;
    ACC.write((char *)&Income, sizeof(Income));

    cout << "Enter the current expenditure: ";
    cin >> Expenditure;
...
void ModifyTransactions()
{
    ...
    while (F.read((char *)&T, sizeof(T)))
    {
        if (TNo == T.RnTransNo())
        {
            ...
            cout << "Transaction Number(Y/N)? ";
            cin >> Toggle;
            if (Toggle == 'Y')
            {
                cout << "Enter the new Transaction Number: ";
                cin >> NewTNo;
                T.CgTransNo(NewTNo);
            }
            cout << "Transaction Name(Y/N)? ";
            cin >> Toggle;
            if (Toggle == 'Y')
            {
                cout << "Enter the new Transaction Name: ";
                gets(NewTName);
                T.CgTransName(NewTName);
            }
            cout << "Transaction Date(Y/N)? ";
            cin >> Toggle;
            if (Toggle == 'Y')
            {
...
void Login(ACCOUNT DB[], int &Access)
{
    ...
    case 2:
        ...
        do
        {
            cout << "Enter the accountant username: ";
            cin >> usrnm;
            for (int I = 0; I < 10; I++)
        ...
        do
        {
            cout << "Enter the accountant password: ";
            Encrypt(pswrd);
            for (int I = 0; I < 10; I++)
